...
#include "sickldmrs/sickldmrs-private.h"
...
sickldmrs_recv_reply(struct sickldmrs_device *dev, struct sickldmrs_header *h)
{
	...
	case SLDMRS_CMD_GET_PARAM:
		...
		uint16_t pIndex,pValue;
		pIndex = p->param_index;
		pValue = p->param_value;
		if(pIndex == 4354){							/* FRECUENCY PARAM */
			dev->scan_frequency = pValue;
		}
		...
		if (dev->debug)
			printf("-- get param: %04x value %04x\n",
